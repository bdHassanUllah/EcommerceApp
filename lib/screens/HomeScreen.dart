...
class HomeScreen extends ConsumerWidget {
    ...
    final List<Widget> pages = [
      Scaffold(
        appBar: AppBar(
          ...
          title: Text(tabTitles[selectedTabIndex], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 25)),
  ...
  Widget _buildPostList(AsyncValue<List<dynamic>> posts) {
    return posts.when(
      data: (data) {
        if (data.isEmpty) {
          return const Center(child: Text("No posts available")); // ✅ Handle empty list
        }
