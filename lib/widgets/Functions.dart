...
class ShareUtil {
  static Future<void> sharePost(BuildContext context, String postId, {required String pageRoute}) async {
    ...
    if (post != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load post URL')),
      );
    }
  }
  ...
  Future<void> sharePost(BuildContext context, String postId, {required String pageRoute}) async {
    // Fetch the correct URL from your API
    final String apiUrl = "https://ecommerce.com.pk/wp-json/api/v1/$pageRoute/$postId";
    final response = await http.get(Uri.parse(apiUrl));

    if (response.statusCode == 200) {
      final Map<String, dynamic> data = json.decode(response.body);
      final String postUrl = data['url']; // Assuming the API returns a 'url' field
